...
Here is an explanation of the `update` command:
****
...
+
image::update/updateUG/UpdateStep1.png[]
+
. When you realize the Laksa is not Curry Laksa and want to change the transaction's name, you type **update i/2 n/Laksa** into the command box and hit kbd:[Enter].
+
image::update/updateUG/UpdateStep2.png[]
+
...
+
image::update/updateUG/UpdateStep3.png[]
...
image::update/updateUG/UpdateStep4.png[]
...
image::update/updateUG/UpdateStep5.png[]
// end::update[]
...
Occasionally, you may want to create multiple copies of a transaction for recurring incomes or expenses, such as a yearly donation or monthly bills. Simply use the `clone` feature to achieve this.

==== Command syntax:
`clone i/INDEX [o/OCCURRENCE]`

The following is an explanation for `clone`:
...
==== Example usage: +
You receive your fixed salary on the 29th of every month. You don't want to tediously add your salary to THRIFT every month, so you add clones of your salary income for the next year at one go.

You do the following on 29 October 2019:

NOTE: The following example uses the displayed list of all existing transactions.

. You have the salary income entry you received on 29/10/2019 in the list. It is at index 4.
+
image::clone/cloneUG/CloneStep1.png[]

. To create clones of the salary entry for 29/10/2019 and the next 11 months up to 29 September 2020, you type **clone i/4 o/monthly:12** and hit kbd:[Enter].
+
image::clone/cloneUG/CloneStep2.png[]

. You can see that 12 clones of the transaction at index 4 are created.
+
image::clone/cloneUG/CloneStep3.png[]
// end::clone[]
